...
class WordPress_Store_Locator_Listing
{
    ...
    public function get_store_locator_listing($atts = array())
    {
        ...
        if($key == "store_category" || $key == "store_filter") {
            $isTax = true;
            $unique_values = $this->get_unique_post_term_values($key, 'stores');
        } else {
            $prefix = "wordpress_store_locator_";
            $key = $prefix . $key;
            $unique_values = $this->get_unique_post_meta_values($key, 'stores');
        }
            ...
            echo '<div class="store-locator-listing-item">';
                ...
                foreach ($posts as $post) {
                    ...
                    if($first_post) {
                        ...
                        $sidebar .= '<a href="#" class="store-locator-listing-post-link ' . $linkClass . '" data-id="' . $unique_value_selector . $post->ID . '" ><li>' . $post->post_title . '</li></a>';
    ...
    public function get_content($post) 
    {
        ...
        $content = $title . $categories . $filter . $content . $address . $contact . $additional_information . $opening_hours . $opening_hours2 . $contactStore . $review . $map;
    ...
    public function get_unique_post_meta_values( $key = '', $type = 'post', $status = 'publish' ) 
    {
        global $wpdb;
        if( empty( $key ) )
            return;
    ...
    public function get_unique_post_term_values($key, $type) 
    {
        global $wpdb;
